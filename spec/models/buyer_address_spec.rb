...
RSpec.describe BuyerAddress, type: :model do
  describe '配送先住所のバリデーション' do
    context '正常系' do
      ...
      it '建物名は任意であること' do
        buyer_address = build(:buyer_address, building: '')
        expect(buyer_address).to be_valid
      end
    end
    ...
    context '異常系' do
      ...
      it '電話番号は10桁以上11桁以内の半角数値のみ保存可能なこと - 9桁の場合' do
        buyer_address = build(:buyer_address, phone_number: '090123456')
        buyer_address.valid?
        expect(buyer_address.errors.full_messages).to include('Phone number is invalid')
      end

      it '電話番号は10桁以上11桁以内の半角数値のみ保存可能なこと - 12桁の場合' do
        buyer_address = build(:buyer_address, phone_number: '090123456789')
        buyer_address.valid?
        expect(buyer_address.errors.full_messages).to include('Phone number is invalid')
      end

      it '電話番号は10桁以上11桁以内の半角数値のみ保存可能なこと - ハイフンが含まれる場合' do
        buyer_address = build(:buyer_address, phone_number: '090-1234-5678')
        buyer_address.valid?
        expect(buyer_address.errors.full_messages).to include('Phone number is invalid')
      end
    end
